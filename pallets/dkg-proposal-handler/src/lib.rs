...
use dkg_runtime_primitives::{
	DKGPayloadKey,
	offchain::storage_keys::{OFFCHAIN_SIGNED_PROPOSALS, SUBMIT_SIGNED_PROPOSAL_ON_CHAIN_LOCK}, OffchainSignedProposals, ProposalAction, ProposalHandlerTrait,
...
pub mod pallet {
	use dkg_runtime_primitives::{DKGPayloadKey, utils::ensure_signed_by_dkg};
				...
				match decode_proposal_identifier(&prop) {
					Ok(v) => {
						Self::deposit_event(Event::<T>::ProposalAdded {
							key: v.key.clone(),
							target_chain: v.target_chain_id.clone(),
...
impl<T: Config> ProposalHandlerTrait for Pallet<T> {
	fn handle_unsigned_proposal(proposal: Vec<u8>, _action: ProposalAction) -> DispatchResult {
		...
		if let Ok(v) = decode_proposal_identifier(&proposal) {
			Self::depoist_event(Event::<T>::ProposalAdded {
				key: v.key.clone(),
				target_chain: v.target_chain_id.clone(),
