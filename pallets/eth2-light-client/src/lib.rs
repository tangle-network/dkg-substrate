...
use eth2_utility::{consensus::*, types::*};
use eth_types::{eth2::*, BlockHeader, H256};
use near_sdk::{
	assert_self,
	collections::{LookupMap, UnorderedMap},
	env, near_bindgen, AccountId, PanicOnDefault,
};
use near_sdk_inner::{collections::LazyOption, Balance, BorshStorageKey, Promise};
...
impl Eth2Client {
	...
	pub fn init(#[serializer(borsh)] args: InitInput) -> Self {
		...
		#[cfg(feature = "mainnet")]
		{
			assert!(args.validate_updates, "The updates validation can't be disabled for mainnet");
	...
	pub fn register_submitter(&mut self) {
		...
		assert!(!self.submitters.contains_key(&account_id), "The account is already registered");
		...
		let refund = amount.checked_sub(self.min_storage_balance_for_submitter).unwrap();
		if refund > 0 {
	...
	pub fn submit_execution_header(&mut self, #[serializer(borsh)] block_header: BlockHeader) {
		...
		if self.finalized_beacon_header.execution_block_hash != block_header.parent_hash {
			self.unfinalized_headers.get(&block_header.parent_hash).unwrap_or_else(|| {
				panic!(
		...
		assert!(insert_result.is_none(), "The block {} already submitted!", &block_hash);
	}
...
impl Eth2Client {
	...
	fn update_finalized_header(&mut self, finalized_header: ExtendedBeaconBlockHeader) {
		...
		loop {
			...
			cursor_header =
				self.unfinalized_headers.get(&cursor_header.parent_hash).unwrap_or_else(|| {
					panic!(
						"Header has unknown parent {:?}. Parent should be submitted first.",
		...
		self.finalized_execution_header.set(&finalized_execution_header_info);
		...
		#[cfg(feature = "logs")]
		env::log_str(
			format!("Finish update finalized header. Used gas: {}", env::used_gas().0).as_str(),
		);
	...
	fn commit_light_client_update(&mut self, update: LightClientUpdate) {
		...
		if update_period == finalized_period + 1 {
			self.current_sync_committee.set(&self.next_sync_committee.get().unwrap());
	...
	fn update_submitter(&mut self, submitter: &AccountId, value: i64) {
		...
		self.submitters.insert(submitter, &num_of_submitted_headers.try_into().unwrap());
	}
