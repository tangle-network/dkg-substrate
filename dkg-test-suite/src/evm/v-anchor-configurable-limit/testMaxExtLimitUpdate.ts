...
async function testMaxExtLimitUpdateProposal() {
	const api = await ApiPromise.create({provider});
	await registerResourceId(api);
	await sendMaxExtLimitUpdateProposal(api);
	...
	const proposalType = {maxextlimitupdateproposal: vAnchorConfigurableLimitProposal.header.nonce}
...
async function sendMaxExtLimitUpdateProposal(api: ApiPromise) {
	...
	const kind = api.createType('DkgRuntimePrimitivesProposalProposalKind', 'MaxExtLimitUpdate');
