...
it('should be able to sign token add & remove proposal', async () => {
  ...
  const tokenToAdd = await MintableToken.createToken('testToken', 'TEST', wallet1);
  {
    ...
    await waitForEvent(polkadotApi, 'dKGProposalHandler', 'ProposalSigned', { key: 'TokenAddProposal' });
    ...
    expect((await governedToken.contract.getTokens()).includes(tokenToAdd.contract.address)).to.eq(true);
  }
  ...
  // now we need to wait until the proposal to be signed on chain.
  await waitForEvent(polkadotApi, 'dKGProposalHandler', 'ProposalSigned', { key: 'TokenRemoveProposal' });
  // now we need to query the proposal and its signature.
  const key = {
  ...
  await tx2.wait();
  // Want to check that token was actually added
  expect((await governedToken.contract.getTokens()).includes(tokenToAdd.contract.address)).to.eq(false);
});
