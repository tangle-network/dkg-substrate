...
it('should be able to sign resource id update proposal', async () => {
  ...
  await waitForEvent(polkadotApi, 'dKGProposalHandler', 'ProposalSigned', { key: 'ResourceIdUpdateProposal' });
  ...
  expect(await bridgeSide.contract._resourceIDToHandlerAddress(newResourceId)).to.eq(handlerAddress);
});
