...
import {MaxDepositLimitProposal, signAndSendUtil} from '../src/evm/util/utils';
...
import { executeAfter } from './utils/util';
		...
		const proposalPayload: MaxDepositLimitProposal = {
			header: {
				...
				functionSignature: encodeFunctionSignature(
					vAnchor.contract.interface.functions['configueMaximumDepositLimit(uint256)'].format()
				),
				nonce: 1,
				chainId: localChain.chainId,
                chainIdType: ChainIdType.EVM
			},
			maxDepositLimitBytes: "0x50",
		};
