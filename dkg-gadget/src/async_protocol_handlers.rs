...
// AsyncProtocol::new(state_machine, IncomingAsyncProtocolWrapper { receiver: rx }, OutgoingAsyncProtocolWrapper {. })
...
pub trait TransformIncoming {
    fn transform(self) -> Result<Msg<Self>, DKGError> where Self: Sized;
}

pub trait TransformOutgoing {
    fn transform(self) -> Result<Self, DKGError> where Self: Sized;
}
