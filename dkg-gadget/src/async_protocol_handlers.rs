...
pub mod meta_channel {
	...
	impl<B, BE, C> MetaDKGMessageHandler<B, BE, C> where
		...
		fn new_inner<SM: StateMachineIface>(sm: SM, params: AsyncProtocolParameters<B, C>, channel_type: ProtocolType) -> Result<Self, DKGError>
			...
			let outgoing_to_wire = Self::generate_outgoing_to_wire_fn::<SM>(current_authority_set, best_authorities.clone(), autority_public_key.clone(), gossip_engine, keystore, outgoing_rx, channel_type.clone());
		...
		fn new_voting(params: AsyncProtocolParameters<B, C>) -> Result<Self, DKGError> {
			let protocol = Box::pin(async move {
				...
				let verify_fn = Self::generate_verification_function(params.client.clone(), params.latest_header.clone(), params.best_authorities.clone(), params.authority_public_key.clone());
				let mut incoming_wrapper = IncomingAsyncProtocolWrapper::new(params.signed_message_receiver, ProtocolType::Voting, verify_fn);
				...
				let (party_ind, round_id, id) = Self::get_party_round_id(&params.best_authorities, &params.authority_public_key, &params.current_validator_set, &params.keystore);
		...
		fn get_party_round_id(best_authorities: &Vec<Public>, authority_public_key: &Public, current_authority_set: &Arc<RwLock<AuthoritySet<Public>>>, keystore: &DKGKeystore) -> (Option<u16>, AuthoritySetId, Public) {
			let party_ind = find_index::<AuthorityId>(best_authorities, authority_public_key).map(|r| r as u16 + 1);
		...
		fn generate_outgoing_to_wire_fn<SM: StateMachineIface>(current_authority_set: Arc<RwLock<AuthoritySet<Public>>>, current_authorities: Vec<Public>, authority_public_key: Public, gossip_engine: Arc<Mutex<GossipEngine<B>>>, keystore: DKGKeystore, mut outgoing_rx: UnboundedReceiver<Msg<<SM as StateMachine>::MessageBody>>, proto_ty: ProtocolType) -> Pin<Box<dyn SendFuture>>
			where <SM as StateMachine>::MessageBody: Serialize,
			 ...
			 Box::pin(async move {
				...
				while let Some(unsigned_message) = outgoing_rx.next().await {
					...
					let (_, round_id, id) = Self::get_party_round_id(&current_authorities, &authority_public_key, &current_authority_set, &keystore);
