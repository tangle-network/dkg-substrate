	...
	<SM as StateMachine>::Output: Send,
{
	...
	let async_proto = Box::pin(async move {
		...
		SM::on_finish(res, params_for_end_of_proto, additional_param, async_index).await
	});

	// For taking all unsigned messages generated by the AsyncProtocols, signing them,
	// and thereafter sending them outbound
	let outgoing_to_wire =
		generate_outgoing_to_wire_fn::<SM, BI>(params.clone(), outgoing_rx, channel_type.clone(), async_index);
