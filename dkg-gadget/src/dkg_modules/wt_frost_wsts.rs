...
pub async fn run_dkg<RNG: RngCore + CryptoRng, Net: NetListen>(
	signer: &mut v2::Party,
	rng: &mut RNG,
	net: &Net,
	n_signers: usize,
	...
	while received_shares.len() < n_signers {
		match net.next_message().await {
			Some(FrostMessage::DKG { party_id, shares, key_ids, poly_commitment }) => {
				received_shares.insert(party_id, shares);
				received_key_ids.insert(party_id, key_ids);
				received_poly_commitments.insert(party_id, poly_commitment);
			},
...
pub async fn run_signing<RNG: RngCore + CryptoRng, Net: NetListen>(
	signer: &mut v2::Party,
	rng: &mut RNG,
	msg: &[u8],
	net: &Net,
	...
	while party_nonces.len() < n_signers {
		match net.next_message().await {
			Some(FrostMessage::Sign { party_id: party_id_recv, key_ids, nonce }) => {
	...
	// Sort the vecs all in order
	//let party_ids = party_ids.into_iter().sorted().collect_vec();
	...
	while signature_shares.len() < n_signers {
		match net.next_message().await {
			Some(FrostMessage::SignFinal { party_id, signature_share }) => {
...
pub trait NetListen {
	...
	async fn next_message(&self) -> Option<FrostMessage>;
	async fn send_message(&self, msg: FrostMessage) -> Result<(), Self::Error>;
}

#[cfg(test)]
mod tests {
	use crate::dkg_modules::wt_frost_wsts::{FrostMessage, NetListen};
	...
	struct TestNetworkLayer {
		...
		rx: Arc<tokio::sync::Mutex<tokio::sync::broadcast::Receiver<FrostMessage>>>,
	}
	...
	async fn test_inner<const N: u32, const T: u32, const K: u32>() {
		...
		for (party, network) in parties.iter_mut().zip(networks.iter()) {
			dkgs.push(Box::pin(async move {
				let mut rng = rand::thread_rng();
				crate::dkg_modules::wt_frost_wsts::run_dkg(party, &mut rng, network, N as _).await
			}));
		}
		...
		for (party, network) in parties.iter_mut().zip(networks.iter()).take(T as _) {
			let public_key = public_key.clone();
			signers.push(Box::pin(async move {
				let mut rng = rand::thread_rng();
				crate::dkg_modules::wt_frost_wsts::run_signing(
